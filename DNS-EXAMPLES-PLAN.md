...
**Status:** Ready to implement (deps branch pinned)
...
## Temporary deps branch pin

All deps references in this repo currently point at branch **`add-env-suffix-to-dns-modules`** (not `main`) while that branch is under review:

- Terraform module sources: `?ref=add-env-suffix-to-dns-modules`
- GitHub Actions: `agent-0028/deps/github_actions/install-tofu-client@add-env-suffix-to-dns-modules`

**Revert to `main`** once the deps PR is merged.

Files already updated:

- [`terraform/nonprod/main.tf`](terraform/nonprod/main.tf), [`terraform/prod/main.tf`](terraform/prod/main.tf)
- [`terraform/nonprod/example-bucket.tf`](terraform/nonprod/example-bucket.tf), [`terraform/prod/example-bucket.tf`](terraform/prod/example-bucket.tf)
- All six workflows under [`.github/workflows/`](.github/workflows/)
...
- Git module source: `git::https://github.com/agent-0028/deps.git//terraform/modules/<name>?ref=add-env-suffix-to-dns-modules`
...
No changes to `main.tf`, GitHub Actions workflows (beyond the branch pin), or the config module are expected — CI already runs `tofu plan`/`apply` against all `.tf` files in each env directory.

## env-suffix behavior (deps branch `add-env-suffix-to-dns-modules`)
...
- [x] Update DNS modules in `deps` to apply `env-suffix` internally (`add-env-suffix-to-dns-modules`)
- [x] Pin all deps references in this repo to `add-env-suffix-to-dns-modules`
...
- [ ] After deps PR merges: revert all `add-env-suffix-to-dns-modules` refs back to `main`
